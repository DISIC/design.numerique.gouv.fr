<template>
  <Layout class="roles-cles-page">
    <section class="dg-cover dg-cover--linear fr-mb-6w">
      <div class="dg-cover__container fr-mb-1w">
          ...
          <ol class="fr-breadcrumb__list">
            ...
            <li>
              <span aria-current="page"
                >{{ $page.entretien.firstName }}
                {{ $page.entretien.lastName }}</span
              >
            </li>
          </ol>
        </nav>
        <div class="dg-cover__title fr-grid-row">
          ...
          <div class="fr-col">
            <h1>
              Entretien avec {{ $page.entretien.firstName }}
              {{ $page.entretien.lastName }}
            </h1>

            <p class="fr-text--lead">
              <span v-html="$page.entretien.job" /><br /><span
                v-html="$page.entretien.company"
              /><br />
              <g-link
                :href="$page.entretien.rs"
                class="fr-link fr-text--xs"
                target="_blank"
                title="LinkedIn - nouvelle fenêtre"
                >LinkedIn {{ $page.entretien.firstName }}
                {{ $page.entretien.lastName }}</g-link
              >
            </p>
          </div>
        </div>
    ...
    <div class="dg-content fr-px-2w">
      ...
      <a
        href="/articles/2023-10-18-paroles-de-responsables-du-design/"
        class="fr-mt-6w fr-link fr-icon-arrow-left-line fr-link--icon-left"
