  ...
  <Layout>
      ...
      <div class="dg-cover__container fr-mb-1w">
        ...
        <h1 class="dg-cover__title">
          <img
            ...
            aria-hidden="true"
          />Nos formations en {{ $page.formation.nom }}
        </h1>
    ...
    <ul class="dg-content fr-px-0 fr-px-md-2w fr-py-2w">
      <li
        ...
        :key="cours.id"
        class="fr-card fr-card--horizontal-tier fr-enlarge-link fr-mb-2w"
      >
        <div class="fr-card__header">
          <div class="fr-card__img">
            <g-image
              :src="'/assets/images/formation/' + cours.slug + '.png'"
        ...
        <div class="fr-card__body">
          <div class="fr-card__content">
            <h2 class="fr-card__title">
              <g-link
                :to="
                  '/formations/' + $page.formation.slug + '/' + cours.slug + '/'
                "
                class="fr-card__link"
                >{{ cours.nom }}</g-link
              >
            ...
            <div class="fr-card__end">
              <p
                v-show="
                  futurCours.filter((element) => element == cours.id).length > 0
                "
                class="dg-flex-start fr-badge fr-badge--sm fr-badge--new"
              >
                Inscriptions ouvertes
              </p>
              <p
                v-show="
                  futurCours.filter((element) => element == cours.id).length ==
                    0 && cours.replay
                "
                class="dg-flex-start fr-badge fr-badge--sm"
              >
                Replay disponible
              </p>
            </div>
...
export default {
    ...
    return {
      ...
      meta: [
        ...
        },
        {
          property: "og:title",
          content:
            "Formations en " + this.$page.formation.nom + " - DesignGouv",
        },
        {
  ...
  computed: {
    sortedCours: function () {
      return this.$page.formation.cours
        .filter((cours) => cours.publier)
