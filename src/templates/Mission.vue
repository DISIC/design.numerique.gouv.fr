  ...
  <Layout showServices>
    <div class="dg-cover dg-cover--linear fr-mb-6w">
      <div class="dg-cover__container fr-mb-1w">
        ...
        <p class="fr-text--lead">
          Cette mission est menée dans le cadre de notre accompagnement
    ...
    <div class="dg-content dg-content--xs fr-px-0 fr-px-md-2w">
      <div class="detail fr-mb-1w">
        <span class="dg-picto dg-picto--sm fr-mr-1w"
          ><font-awesome
            :icon="['fas', 'route']"
            height="16px"
            aria-hidden="true"
        ...
        <p
          v-if="$page.mission.status == 'futur'"
          class="fr-badge fr-badge--pink-macaron"
        >
          À venir
        </p>
        <p
          v-else-if="$page.mission.status == 'present'"
          class="fr-badge fr-badge--green-tilleul-verveine"
        >
          En cours
        </p>
        <p
          v-else-if="$page.mission.status == 'past'"
          class="fr-badge fr-badge--grey"
        >
          Terminée
        </p>
      </div>
      <div class="detail fr-mb-1w">
        ...
        <ul class="detail__data detail__data--procedures">
          <li v-for="procedure in $page.mission.procedures">
            <span v-if="procedure.url"
              ><a
                :href="procedure.url"
                :title="procedure.name + ' - Nouvelle fenêtre'"
                target="_blank"
                >{{ procedure.name }}</a
              ></span
            >
      ...
      <div
        v-if="$page.mission.goals.length"
        class="detail detail--goals fr-mb-1w"
      >
        <span class="dg-picto dg-picto--sm fr-mr-1w"
          ><font-awesome
            :icon="['fas', 'tasks']"
        ...
        <ol
          class="detail__data detail__data--goals fr-ml-2w fr-ml-md-6w fr-mb-1v"
        >
          <li v-for="goal in $page.mission.goals" class="goal fr-mb-1v">
            <span
              v-if="goal.done"
              class="goal__status goal__status--ok fr-mr-1w"
              ><font-awesome
                :icon="['fas', 'check']"
                width="16"
                height="16"
                title="Atteint :"
            /></span>
            <span v-else class="goal__status fr-mr-1w"
              ><font-awesome
                :icon="['fas', 'check']"
                width="16"
                height="16"
                title="Pas encore atteint :"
            /></span>
      ...
      <div v-if="$page.mission.jobs.length" class="detail fr-mb-1w">
        ...
        <p class="detail__data">
          <span v-if="$page.mission.direction"
            >{{ $page.mission.department }} — {{ $page.mission.direction }} —
            {{ $page.mission.place }}</span
          >
          <span v-else
            >{{ $page.mission.department }} — {{ $page.mission.place }}</span
          >
        </p>
      </div>
      <div
        v-if="$page.mission.team.length"
        class="detail detail--team fr-mb-1w"
      >
        <span class="dg-picto dg-picto--sm fr-mr-1w"
          ><font-awesome
        ...
        <ul
          class="detail__data detail__data--team fr-ml-2w fr-ml-md-6w fr-mb-1v"
        >
          <li
            v-for="member in $page.mission.team"
            :key="member.id"
            class="team-member fr-mb-1v"
          >
            <g-image
              :src="member.photo"
              class="team-member__photo fr-mr-1w"
              alt=""
            />
      ...
      <div v-if="$page.mission.jobs.length" class="detail detail--job fr-mb-1w">
        ...
        <ul
          class="detail__data detail__data--job fr-ml-2w fr-ml-md-6w fr-mb-1v"
        >
          <li
            v-for="job in $page.mission.jobs"
            :key="job.id"
            class="team-member fr-mb-1v"
          >
            <span
              class="dg-picto dg-picto--xs dg-picto--pink dg-text-bold fr-mr-1w"
              >{{ job.count }}</span
            >
      ...
      meta: [
        {
          name: "description",
          content:
            "Une équipe d'experts du Commando UX est mobilisée sur cette mission",
        },
        {
          property: "og:title",
        ...
        },
        {
          property: "og:image",
          content:
            "https://design.numerique.gouv.fr/assets/meta-images/CUX.png",
        },
      ],
    };
  },
